...
-- ✅ FIXED: Combined Script Hub with Working Pet Detection (Tool or Hand-Held)
-- Now supports Grow a Garden's pet holding system.
...
-- ✅ NEW: Detect held pet in tools or hand
...
local function applyMutationToHeldPet()
    ...
    if not pet then
        warn("❌ No pet found in hand/tool.")
        PetNameLabel.Text = "No Pet Found"
        return
    end
    ...
    if not part then warn("❌ No base part found.") return end
